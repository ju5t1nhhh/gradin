...
<script>
    ...
    import {delAdmin} from '@/api/getData'
    ...
    export default {
    	data(){
    		return {
		        ...
		        // rules: {
				// 	loginId: [
				// 		{ required: true, message: '请输入新登录ID', trigger: 'blur' },
				// 	],
				// 	pwd: [
    	...
    	methods: {
		    submitForm(formName) {
				this.$refs[formName].validate(async (valid) => {
					if (valid) {
						try{
							let result = await delAdmin(this.formData.loginId);
							if (result.code == 200) {
								this.$message({
					            	type: 'success',
					            	message: '删除成功'
