        ...
        <el-row style="margin-top: 20px;">
  			<el-col :span="12" :offset="4">
		        <el-form :model="formData" :rules="rules" ref="formData" label-width="110px" class="demo-formData">
					<el-form-item label="所在板块" prop="section">
						<el-input v-model="formData.section"></el-input>
					</el-form-item>
					<el-form-item label="待删除敏感词" prop="word">
    ...
    export default {
    	data(){
    		return {
		        ...
		        rules: {
					section: [
						{ required: true, message: '请输入待删除敏感词所在板块', trigger: 'blur' },
					],
					word: [
						{ required: true, message: '请输入待删除敏感词', trigger: 'blur' },
                    ],
				},
			    // baseUrl,
			    // baseImgPath,
    	...
    	methods: {
		    submitForm(formName) {
				this.$refs[formName].validate(async (valid) => {
					if (valid) {
						try{
							let result = await delSW(this.formData);
							if (result.code == 200) {
								this.$message({
					            	type: 'success',
					            	message: '删除成功'
